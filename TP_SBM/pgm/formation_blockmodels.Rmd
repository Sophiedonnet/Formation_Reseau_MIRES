...
```
"SBM"" indicates that the adjacency is not symmetric (undirected network), it is important to specify it for statistical reasons. If the matrix is symmetric (undirected network), we use BM_bernoulli("SBM_sym",tree_bin) instead.
...
```{r plot BM network  poisson, echo=TRUE, eval = FALSE}
...
```{r  tree fungis plot poiss, echo=TRUE} 
